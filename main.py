...
def parse_args():
	...
	p.add_argument("--config-dir", default="config", help="Directory containing main_*.yaml")
	p.add_argument("--verbose", action="store_true", help="Debug log")
	p.add_argument("--log-level", default="", help="Override log level (debug/info/warning/error)")
	return p.parse_args()
...
def main():
	...
	try:
		...
		output_pixel_format = _normalize_pixel_format(getattr(cfg.camera, "output_pixel_format", "bgr8"))
	...
	tcp_info = f"{cfg.comm.tcp.host}:{cfg.comm.tcp.port}" if cfg.trigger.tcp.enabled else "off"
	modbus_info = f"{cfg.comm.modbus.host}:{cfg.comm.modbus.port}" if (cfg.output.enable_modbus or cfg.trigger.modbus.enabled) else "off"
	logging.info(
		"Starting: camera=%s http=%s tcp=%s modbus=%s runtime=%s",
		cfg.camera.type,
		f"{cfg.comm.http.host}:{cfg.comm.http.port}" if cfg.output.enable_http else "off",
		tcp_info,
		modbus_info,
		f"{cfg.runtime.max_runtime_s}s" if cfg.runtime.max_runtime_s else "unlimited",
	)
	logging.info("Config files: main=%s detect=%s", cfg.paths.get("main"), cfg.paths.get("detect"))
	...
	try:
		with camera.session():
			...
			if cfg.trigger.tcp.enabled:
				def on_trigger(addr):
					runtime.app_context.trigger_gateway.report_raw_trigger("TCP", remote_ip=addr)
			...
			runtime.run(runtime_limit_s=cfg.runtime.max_runtime_s if cfg.runtime.max_runtime_s > 0 else None)
		logging.info("Done")
