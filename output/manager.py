...
class ResultStore:
	...
	def __init__(self, base_dir: str, max_records: int = 10, write_csv: bool = True):
		...
		self._write_queue: Optional[queue.Queue[OutputRecord]] = queue.Queue() if write_csv else None
		self._writer_stop = threading.Event()
		self._writer_thread = threading.Thread(target=self._writer_loop, daemon=True) if write_csv else None
		if write_csv:
	...
	def stop(self, join_timeout: float = 1.0):
		...
		if self._write_queue:
			...
			while self._write_queue.unfinished_tasks > 0 and time.perf_counter() < deadline:
				time.sleep(0.05)
	...
	def _append_csv(self, rec: OutputRecord):
		...
		with open(self.csv_path, "a", encoding="utf-8") as f:
			if write_header:
				f.write("id,trigger_date,trigger_time,save_finish_time,result,result_code,duration_ms,remark\n")
			f.write(f"{rec.trigger_seq},{t_date},{t_time},{save_time},{rec.result},{rec.result_code or ''},{(rec.duration_ms or 0.0):.3f},{rec.remark}\n")
