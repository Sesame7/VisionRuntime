...
class _ApiServer:
	def __init__(self, host: str, port: int, context: AppContext, index_path: str, task_reg=None):
		self.host = host
		self.port = port
		...
		def _serialize_record(rec):
			def _dt(val):
				if isinstance(val, datetime):
					ref = val.astimezone(timezone.utc) if val.tzinfo else val.replace(tzinfo=timezone.utc)
...
class HmiOutput:
	def __init__(self, host: str, port: int, context: AppContext, index_path: str, task_reg=None):
		self.server = _ApiServer(host, port, context, index_path=index_path, task_reg=task_reg)
		self._started = False
