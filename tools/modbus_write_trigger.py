...
def main():
	...
	p.add_argument("--coil-offset", type=int, default=800, help="Trigger coil offset (PDU 0-based)")
	p.add_argument("--value", type=int, choices=[0, 1], default=None, help="Write fixed value (0/1)")
	args = p.parse_args()
	...
	with ModbusTcpClient(host=args.host, port=args.port) as client:
		...
		value = args.value
		if value is None:
			read_res = client.read_coils(address=args.coil_offset, count=1, device_id=args.device_id)
			if read_res.isError():
		...
		res = client.write_coil(address=args.coil_offset, value=bool(value), device_id=args.device_id)
		if res.isError():
