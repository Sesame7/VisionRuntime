...
class SimState:
	...
	def set_result_regs(self, trig_time: datetime, seq: int, result_code: int, error_code: int, cycle_ms: int) -> None:
		values = [
			int(trig_time.year),
			int(trig_time.month),
			int(trig_time.day),
...
async def _emit_result(state: SimState, result: str, process_ms: int, trig_time: datetime, seq: int):
	...
	LOG.info("result=%s seq=%s code=%s err=%s cycle_ms=%s", result, seq, result_code, error_code, cycle_ms)
...
async def _run_server(args):
	...
	tasks = [
		...
		asyncio.create_task(_trigger_loop(state, args.result, args.process_ms, args.poll_ms)),
	]
	LOG.info(
...
def _parse_args():
	...
	p.add_argument("--heartbeat-ms", type=int, default=1000, help="Heartbeat toggle interval (ms)")
	p.add_argument("--poll-ms", type=int, default=20, help="CMD_TRIG_TOGGLE polling interval (ms)")
	p.add_argument("--process-ms", type=int, default=80, help="Simulated processing time (ms)")
	p.add_argument("--result", default="OK", choices=RESULT_CHOICES, help="Fixed result mode value")
...
def main():
	...
	logging.basicConfig(level=level_map.get(args.log_level, logging.INFO), format="%(asctime)s [%(levelname)s] %(message)s")
	try:
