...
def detect_overexposure(img: np.ndarray, threshold: int = 245, ratio_threshold: float = 0.02, return_overlay: bool = True) -> Tuple[float, bool, np.ndarray | None]:
	"""
	...
	if img.ndim == 2:
		...
		gray_u16 = (img_u16[:, :, 0] * 77 + img_u16[:, :, 1] * 150 + img_u16[:, :, 2] * 29) >> 8
	...
	overlay[..., 2][over_idx] = (overlay[..., 2][over_idx].astype(np.uint16) + 255) // 2  # R channel in BGR -> index 2
