...
class Detector(Protocol):
	def detect(self, img: np.ndarray) -> Tuple[bool, str, np.ndarray | None, str | None]:
...
def create_detector(name: str, params: dict, *, generate_overlay: bool = True, input_pixel_format: str | None = None) -> Detector:
	...
	if name not in _registry:
		...
		raise ValueError(f"Unknown detector impl '{name}'. Available: {', '.join(_registry.keys()) or 'none'}{hint}")
...
def encode_image_jpeg(img: np.ndarray, quality: int = 50, subsampling: int = 2) -> Tuple[bytes, str]:
	"""
