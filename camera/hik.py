...
def _setup_sdk():
	funcs = {
		...
		"MV_CC_OpenDevice": ((ctypes.c_void_p, ctypes.c_uint32, ctypes.c_uint16), ctypes.c_uint),
		...
		"MV_CC_GetImageBuffer": ((ctypes.c_void_p, ctypes.c_void_p, ctypes.c_uint), ctypes.c_uint),
...
class MvCamera:
	...
	def enum_devices(tlayer_type, dev_list):
		return _sdk.MV_CC_EnumDevices(ctypes.c_uint(tlayer_type), ctypes.byref(dev_list))

	def create_handle(self, dev_info):
		return _sdk.MV_CC_CreateHandle(ctypes.byref(self.handle), ctypes.byref(dev_info))
...
def _output_pixel_type(cfg: CameraConfig) -> int:
	return MV_GVSP_PIX_MONO8 if str(cfg.output_pixel_format).lower() == "mono8" else MV_GVSP_PIX_BGR8_PACKED
...
def _save_image(cam: MvCamera, path: str, dst_type: int, buf, buf_len: int, width: int, height: int) -> int:
...
def _convert_frame(cam: MvCamera, frame: MV_FRAME_OUT, cfg: CameraConfig) -> Tuple[Optional[Tuple[object, int, int, int]], Optional[str]]:
...
def _process_frame(
	...
	if cfg.save_images:
		...
		save_ret = _save_image(cam, path, _output_pixel_type(cfg), buf, buf_len, width, height)
		if save_ret != MV_OK:
			path = None
	...
	return (path if (cfg.save_images and save_ret == MV_OK) else None, arr, now_utc), None
	...
	def session(self):
		...
		try:
			...
			if self.cfg.device_index >= device_list.nDeviceNum:
				raise RuntimeError(f"Device index {self.cfg.device_index} out of range (found {device_list.nDeviceNum})")

			device_info = ctypes.cast(device_list.pDeviceInfo[self.cfg.device_index], ctypes.POINTER(MV_CC_DEVICE_INFO)).contents
			L.info("Camera [%d] %s", self.cfg.device_index, _device_summary(device_info))
