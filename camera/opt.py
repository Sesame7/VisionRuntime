...
def _load_sdk():
	if system == "Windows":
		...
		lib_path = os.path.join(sdk_path, 'SciCamSDK.dll') if sdk_path else os.path.join(_SCRIPT_DIR, 'SciCamSDK.dll')
	elif system == "Darwin":
...
class _SCI_DEVICE_GIGE_INFO_(ctypes.Structure):
	_fields_ = [
		('status', ctypes.c_ubyte), ('name', ctypes.c_ubyte * 64),
		('manufactureName', ctypes.c_ubyte * 32), ('modelName', ctypes.c_ubyte * 32),
		('version', ctypes.c_ubyte * 32), ('userDefineName', ctypes.c_ubyte * 16),
		('serialNumber', ctypes.c_ubyte * 16), ('mac', ctypes.c_ubyte * 6),
		('ip', ctypes.c_uint), ('mask', ctypes.c_uint), ('gateway', ctypes.c_uint),
		('adapterIp', ctypes.c_uint), ('adapterMask', ctypes.c_uint),
...
class _SCI_DEVICE_USB3_INFO_(ctypes.Structure):
	_fields_ = [
		('status', ctypes.c_ubyte), ('name', ctypes.c_ubyte * 64),
		('manufactureName', ctypes.c_ubyte * 64), ('modelName', ctypes.c_ubyte * 64),
		('version', ctypes.c_ubyte * 64), ('userDefineName', ctypes.c_ubyte * 64),
		('serialNumber', ctypes.c_ubyte * 64), ('guid', ctypes.c_ubyte * 64),
		('U3VVersion', ctypes.c_ubyte * 64), ('GenCPVersion', ctypes.c_ubyte * 64)]


class _SCI_DEVICE_CL_INFO_(ctypes.Structure):
	_fields_ = [
		('cardStatus', ctypes.c_ubyte), ('cardName', ctypes.c_ubyte * 64),
		('cardManufacture', ctypes.c_ubyte * 64), ('cardModel', ctypes.c_ubyte * 64),
		('cardVersion', ctypes.c_ubyte * 64), ('cardUserDefineName', ctypes.c_ubyte * 64),
		('cardSerialNumber', ctypes.c_ubyte * 64), ('cameraStatus', ctypes.c_ubyte),
		('cameraType', ctypes.c_ubyte), ('cameraBaud', ctypes.c_uint),
		('cameraManufacture', ctypes.c_ubyte * 64), ('cameraFamily', ctypes.c_ubyte * 64),
		('cameraModel', ctypes.c_ubyte * 64), ('cameraVersion', ctypes.c_ubyte * 64),
		('cameraSerialNumber', ctypes.c_ubyte * 64), ('cameraSerialPort', ctypes.c_ubyte * 64),
...
class SCI_DEVICE_INFO_INFO_UNION(ctypes.Union):
	_fields_ = [
		("gigeInfo", _SCI_DEVICE_GIGE_INFO_), ("usb3Info", _SCI_DEVICE_USB3_INFO_),
		("clInfo", _SCI_DEVICE_CL_INFO_), ("cxpInfo", _SCI_DEVICE_CXP_INFO_)]


class _SCI_DEVICE_INFO_(ctypes.Structure):
	_fields_ = [
		('tlType', ctypes.c_int), ("devType", ctypes.c_int),
		("reserve", ctypes.c_ubyte * 256), ("info", SCI_DEVICE_INFO_INFO_UNION)]
...
class _SCI_CAM_IMAGE_ATTRIBUTE_(ctypes.Structure):
	_fields_ = [
		("width", ctypes.c_uint64), ("height", ctypes.c_uint64),
		("offsetX", ctypes.c_uint64), ("offsetY", ctypes.c_uint64),
		("paddingX", ctypes.c_uint64), ("paddingY", ctypes.c_uint64),
		("pixelType", ctypes.c_int), ("reserve", ctypes.c_ubyte * 32)]


class _SCI_CAM_PAYLOAD_ATTRIBUTE_(ctypes.Structure):
	_fields_ = [
		("frameID", ctypes.c_uint64), ("isComplete", ctypes.c_bool),
		("hasChunk", ctypes.c_bool), ("timeStamp", ctypes.c_uint64),
		("payloadMode", ctypes.c_int), ("imgAttr", _SCI_CAM_IMAGE_ATTRIBUTE_),
...
def _setup_sdk():
	funcs = {
		'SciCam_DiscoveryDevices': ((ctypes.POINTER(SCI_DEVICE_INFO_LIST), ctypes.c_uint), ctypes.c_uint),
		'SciCam_CreateDevice': ((ctypes.c_void_p, ctypes.POINTER(SCI_DEVICE_INFO)), ctypes.c_uint),
		...
		'SciCam_Grab': ((ctypes.c_void_p, ctypes.POINTER(ctypes.c_void_p)), ctypes.c_uint),
		'SciCam_FreePayload': ((ctypes.c_void_p, ctypes.c_void_p), ctypes.c_uint),
		'SciCam_SetCommandValueEx': ((ctypes.c_void_p, ctypes.c_int, ctypes.c_char_p), ctypes.c_uint),
		'SciCam_Payload_GetAttribute': ((ctypes.c_void_p, ctypes.POINTER(SCI_CAM_PAYLOAD_ATTRIBUTE)), ctypes.c_uint),
		'SciCam_Payload_GetImage': ((ctypes.c_void_p, ctypes.POINTER(ctypes.c_void_p)), ctypes.c_uint),
		'SciCam_Payload_ConvertImage': ((ctypes.POINTER(SCI_CAM_IMAGE_ATTRIBUTE), ctypes.c_void_p, ctypes.c_int, ctypes.c_void_p, ctypes.c_void_p, ctypes.c_bool), ctypes.c_uint),
		'SciCam_Payload_SaveImage': ((ctypes.c_char_p, ctypes.c_int, ctypes.c_void_p, ctypes.c_int64, ctypes.c_int64), ctypes.c_uint),
	}
...
class SciCamSDK:
	...
	def discovery(self):
		...
		ret = self.dll.SciCam_DiscoveryDevices(ref(dev_infos), ctypes.c_uint(SciCamTLType.SciCam_TLType_Gige))
	...
	def trigger(self, handle):
		return self.dll.SciCam_SetCommandValueEx(handle, ctypes.c_int(SciCamDeviceXmlType.SciCam_DeviceXml_Camera), b"TriggerSoftware")
	...
	def convert_payload(self, payload, dst_type):
		...
		if not (attr.isComplete and attr.payloadMode == SciCamPayloadMode.SciCam_PayloadMode_2D):
		...
		size = ctypes.c_int(0)
		ret = self.dll.SciCam_Payload_ConvertImage(ref(attr.imgAttr), img_ptr, dst_type, None, ref(size), ctypes.c_bool(True))
		if ret != SCI_CAMERA_OK or size.value <= 0:
			return None, f"convert_size(0x{ret:08X})"

		buf = (ctypes.c_ubyte * size.value)()
		ret = self.dll.SciCam_Payload_ConvertImage(ref(attr.imgAttr), img_ptr, dst_type, buf, ref(size), ctypes.c_bool(True))
		if ret != SCI_CAMERA_OK:
	...
	def save_image(self, path: str, dst_type, buf, width: int, height: int):
		return self.dll.SciCam_Payload_SaveImage(path.encode('utf-8'), dst_type, buf, ctypes.c_int64(width), ctypes.c_int64(height))
...
def _process_payload(
	...
	if cfg.save_images:
		...
		path = os.path.join(target_dir, _format_filename(file_id, cfg.ext, now=now_local))
	...
	return (path if (cfg.save_images and save_ret == SCI_CAMERA_OK) else None, arr, now_utc), None
...
class OptSciCamera(BaseCamera):
	...
	def capture_once(self, idx):
		...
		if last_err:
			L.error("[%s] failed after %d attempts: %s", idx, self.cfg.max_retry_per_frame, last_err)
		else:
	...
	def session(self):
		...
		if self.cfg.device_index >= devs.count:
			raise RuntimeError(f"Device index {self.cfg.device_index} out of range (found {devs.count})")
		...
		_check(SDK.create_device(self.handle, devs.pDevInfo[self.cfg.device_index]), "create")
		try:
			_check(SDK.open_device(self.handle), "open")
			_check(SDK.set_grab_timeout(self.handle, self.cfg.timeout_ms), "set timeout")
