...
def create_camera(name: str, cfg: CameraConfig) -> BaseCamera:
	...
	if name not in _registry:
		...
		raise ValueError(f"Unknown camera type '{name}'. Available: {', '.join(_registry.keys()) or 'none'}{hint}")
...
__all__ = ["CameraConfig", "CaptureResult", "BaseCamera", "register_camera", "create_camera", "ensure_dir"]
