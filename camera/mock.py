...
_ORDER_CHOICES = {"name_asc", "name_desc", "name_natural", "mtime_asc", "mtime_desc", "random"}
...
class MockCamera(BaseCamera):
	...
	def _scan(self):
		if self._order not in _ORDER_CHOICES:
			raise RuntimeError(f"mock order must be one of {sorted(_ORDER_CHOICES)}, got {self._order!r}")
		if self._end_mode not in _END_CHOICES:
			raise RuntimeError(f"mock end_mode must be one of {sorted(_END_CHOICES)}, got {self._end_mode!r}")
