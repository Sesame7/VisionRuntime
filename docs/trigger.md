...
- Thread safety: protect key state such as `trigger_seq` and latest trigger time with a lock; inside the lock, only do decision-making and metadata updates; queue enqueue/overflow handling happens outside the lock to avoid blocking.
