...
- 推荐端口（均可配置，以现场配置文件为准）：
  - 标准端口：`502/tcp`
  - 工程常用：`1502/tcp`（非特权端口）
  - 说明：本项目代码层默认值可配置，当前默认配置常量可能使用其他非特权端口（如 `5020`）；生产部署建议在配置中显式固定为 `502` 或 `1502`
...
当前实现说明：`CMD_RESET` 为“非强一致复位”，复位瞬间若存在在途帧/在途检测，可能仍有少量复位前结果在复位后短时输出。
...
- 复位流程不要求“强一致屏障”；系统不会暂停所有在途处理后再复位。
- 因此 PLC 在复位后的极短窗口内，仍可能观察到少量复位前触发对应的结果更新。
