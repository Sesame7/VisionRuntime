    ...
    def report_raw_trigger(
        ...
        with self._lock:
            ...
            if (
                ...
                and (now - self.last_accept_ts) * 1000 < self.min_interval_ms
            ):
            ...
            if (
                self.debounce_ms
                and (now - self.last_accept_ts) * 1000 < self.debounce_ms
            ):
            ...
            if self._is_low_priority_blocked(now, source):
                L.debug(
                    "Low priority blocked during high priority cooldown: %s", source
                )
                return False
