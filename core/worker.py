...
def make_queue_overflow_record(frame_id: int, source: str, device_id: str, triggered_at: datetime, captured_at: datetime, detected_at: datetime, t0: Optional[float] = None, remark: str = "queue_overflow") -> OutputRecord:
...
class CameraWorker(BaseWorker):
	def __init__(self, camera, id_manager: GlobalIdManager, trigger_queue: queue.Queue, detect_queue: "DetectQueueManager", result_sink: Callable[[OutputRecord, Optional[Tuple[bytes, str]]], None]):
	...
	def run(self):
		while not self._stop_evt.is_set():
			...
			t0 = (t0_val / 1000.0) if isinstance(t0_val, (int, float)) else time.perf_counter()
			...
			try:
				...
				device_id = str(getattr(res, "device_id", "") or getattr(getattr(self.camera, "cfg", None), "device_index", ""))
				captured_at = getattr(res, "captured_at", None) or datetime.now(timezone.utc)
				if not res.success or res.image is None:
					error_msg = res.error or "capture_failed"
					msg = error_msg if error_msg.startswith("Error:") else f"Error: {error_msg}"
...
class DetectQueueManager:
	...
	def enqueue(self, task: AcqTask, result_sink: Callable[[OutputRecord, Optional[Tuple[bytes, str]]], None]):
...
class DetectWorker(BaseWorker):
	def __init__(self, queue_mgr: DetectQueueManager, result_sink: Callable[[OutputRecord, Optional[Tuple[bytes, str]]], None], detector, timeout_ms: float = 2000.0, enable_preview: bool = True):
		...
		while not self._stop_evt.is_set():
			...
			try:
				...
				try:
					...
					ok, message, overlay_img, result_code = False, "Error: detection exception", None, "ERROR"
				...
				preview_bytes = None
				preview_mime = None
				if self.enable_preview and overlay_img is not None and rec.result in ("OK", "NG"):
					try:
						preview_bytes, preview_mime = _encode_image(overlay_img)
					except Exception:
						L.warning("Preview encode failed; disable detect.enable_preview or install dependencies", exc_info=True)
						preview_bytes, preview_mime = None, None
				overlay = (preview_bytes, preview_mime) if (preview_bytes and preview_mime) else None
				# Emit per-frame timing at INFO for both OK/NG; TIMEOUT/ERROR at WARNING.
				log_fn = L.info if rec.result in ("OK", "NG") else L.warning
				log_fn("[%5s] grab=%.2fms detect=%.2fms total=%.2fms result=%s",
					   task.frame_id,
					   (task.grab_ms or 0.0),
					   detect_ms,
