...
from typing import Any, Callable, Coroutine, Optional, Protocol, Tuple, TypeVar, cast, TYPE_CHECKING
...
def run_async(coro: Coroutine[Any, Any, T], timeout: float | None = 0.5) -> T:
	...
	if _loop_thread_ident is not None and threading.get_ident() == _loop_thread_ident:
		raise RuntimeError("run_async must not be called from the loop thread; await directly")
...
class SystemRuntime:
	...
	def __init__(self, app_context: AppContext, camera_worker: CameraWorker, detect_worker: DetectWorker, output_mgr: OutputManager):
	...
	def start(self, trigger: Optional[TriggerHandle] = None, triggers: Optional[list[TriggerHandle]] = None):
	...
	def run(self, runtime_limit_s: float | None = None):
		...
		try:
			while not self._stop_evt.wait(0.1):
				if runtime_limit_s is not None and (time.perf_counter() - start_ts) >= runtime_limit_s:
					L.info("Runtime limit reached (%ss); shutting down service", runtime_limit_s)
					self.request_stop()
		finally:
			self.stop()
...
def build_runtime(
	...
	result_store = ResultStore(base_dir=save_dir, max_records=history_size, write_csv=write_csv)
	...
	app_context = AppContext(
		trigger_gateway=TriggerGateway(
			...
			min_interval_ms=float(getattr(trigger_cfg, "global_min_interval_ms", 0.0) or 0.0),
			high_priority_cooldown_ms=float(getattr(trigger_cfg, "high_priority_cooldown_ms", 0.0) or 0.0),
			high_priority_sources=set(getattr(trigger_cfg, "high_priority_sources", []) or []),
			low_priority_sources=set(getattr(trigger_cfg, "low_priority_sources", []) or []),
	...
	if enable_http:
		...
		hmi_output = HmiOutput(http_host, http_port, app_context, index_path=index_path, task_reg=output_mgr.register_task)
		output_mgr.add_channel(hmi_output)
	modbus_io_enabled = bool(enable_modbus if enable_modbus_io is None else enable_modbus_io)
	...
	camera_worker = CameraWorker(camera, id_mgr, trigger_queue, queue_mgr, result_sink=output_mgr.publish)
	detect_worker = DetectWorker(queue_mgr, result_sink=output_mgr.publish, detector=detector, timeout_ms=detect_timeout_ms, enable_preview=enable_preview)
