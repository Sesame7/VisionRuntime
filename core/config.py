...
def load_config(config_dir: str = "config") -> LoadedConfig:
    ...
    camera = _build_dataclass(
        CameraConfigBlock, main_data.get("camera", {}), main_path, section="camera"
    )
    ...
    output = _build_dataclass(
        OutputConfigBlock, main_data.get("output", {}), main_path, section="output"
    )
...
def _build_dataclass(cls, data: Dict[str, Any], main_path: str, section: str):
    ...
    for k, v in (data or {}).items():
        if hasattr(obj, k):
            ...
            logging.warning(
                "Unknown field %s.%s in %s (ignored)", section, k, main_path
            )
    return obj
...
def _build_trigger_config(data: Dict[str, Any], main_path: str) -> TriggerConfigBlock:
    ...
    if isinstance(tcp_data, dict):
        cfg.tcp = _build_dataclass(
            TriggerTcpConfigBlock, tcp_data, main_path, section="trigger.tcp"
        )
    modbus_data = data.get("modbus")
    if isinstance(modbus_data, dict):
        cfg.modbus = _build_dataclass(
            TriggerModbusConfigBlock, modbus_data, main_path, section="trigger.modbus"
        )
...
def _build_comm_config(data: Dict[str, Any], main_path: str) -> CommConfigBlock:
    ...
    if isinstance(tcp_data, dict):
        cfg.tcp = _build_dataclass(
            CommTcpConfigBlock, tcp_data, main_path, section="comm.tcp"
        )
    modbus_data = data.get("modbus")
    if isinstance(modbus_data, dict):
        cfg.modbus = _build_dataclass(
            CommModbusConfigBlock, modbus_data, main_path, section="comm.modbus"
        )
    http_data = data.get("http")
    if isinstance(http_data, dict):
        cfg.http = _build_dataclass(
            CommHttpConfigBlock, http_data, main_path, section="comm.http"
        )
    return cfg
...
def _import_modules(imports: Any, main_path: str):
    ...
    for path in imports:
        ...
        try:
            ...
            raise ConfigError(
                f"Failed to import {path!r} declared in {main_path}: {e}"
            ) from e
