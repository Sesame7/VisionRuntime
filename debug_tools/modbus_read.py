...
def main():
    ...
    p.add_argument(
        "--di-count", type=int, default=6, help="Number of discrete inputs to read"
    )
    p.add_argument(
        "--ir-offset",
        type=int,
    ...
    with ModbusTcpClient(host=args.host, port=args.port) as client:
        ...
        coil_res = client.read_coils(
            address=args.coil_offset, count=args.coil_count, device_id=args.device_id
        )
        di_res = client.read_discrete_inputs(
            address=args.di_offset, count=args.di_count, device_id=args.device_id
        )
